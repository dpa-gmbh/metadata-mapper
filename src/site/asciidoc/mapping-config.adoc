...
When using an output character encoding like ISO-8859-1 not all characters of the input document may
be correctly transfered to the image meta data fields. For this case a character mapping can be declared.
...
`<iim>` and `<xmp>` define the character encoding for the related outputs. Please refer to
<<sec.characterMapping>>.
